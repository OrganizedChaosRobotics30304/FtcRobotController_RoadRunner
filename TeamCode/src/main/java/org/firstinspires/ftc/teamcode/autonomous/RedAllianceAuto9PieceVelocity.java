...
@Override
  public void runOpMode(){
    ...
    TrajectoryActionBuilder moveToShootFirstRow = moveToIntakeFirstRow.endTrajectory().fresh()
            ...
            .splineToLinearHeading(new Pose2d(55, 9, Math.toRadians(347)),Math.PI / 2);
    ...
    TrajectoryActionBuilder moveToShootSecondRow = moveToIntakeSecondRow.endTrajectory().fresh()
            .setTangent(Math.toRadians(270))
            .splineToLinearHeading(new Pose2d(55, 3, Math.toRadians(347)),Math.PI / 2);
    ...
    Action shooterAndPassthroughPreLoad = new ParallelAction(
            shooter.runShooterPreload(2.6),
            new SequentialAction(
                    new SleepAction(0.7),
                    passthrough.runPassthrough(1.9)),
            new SequentialAction(
            new SleepAction(0.7),
            intake.runIntake(1.9)
    )
    );

    Action shooterAndPassthroughFirstRow = new ParallelAction(
            shooter.runShooterPickUps(2.6),
            new SequentialAction(
                    new SleepAction(0.7),
                    passthrough.runPassthrough(1.9)
            ),
